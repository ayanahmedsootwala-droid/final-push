...
export default function SellCarPage() {
  ...
  const [dbVariants, setDbVariants] = useState<{ id: string; name: string }[]>([]);
  ...
  useEffect(() => {
    ...
    supabase.from('car_brands').select('id, name').eq('is_active', true).order('name')
      .then(({ data }) => { setDbBrands(data ?? []); setLoadingBrands(false); });
  }, []);

  useEffect(() => {
    if (!form.brand_name) { setDbModels([]); setDbVariants([]); return; }
    const brand = dbBrands.find(b => b.name === form.brand_name);
    if (!brand) return;
    setLoadingModels(true); setDbModels([]); setDbVariants([]);
    supabase.from('car_models').select('id, name').eq('brand_id', brand.id).eq('is_active', true).order('name')
      .then(({ data }) => { setDbModels(data ?? []); setLoadingModels(false); });
  }, [form.brand_name, dbBrands]);

  useEffect(() => {
    if (!form.model_name) { setDbVariants([]); return; }
    const model = dbModels.find(m => m.name === form.model_name);
    if (!model) return;
    setLoadingVariants(true); setDbVariants([]);
    supabase.from('car_variants').select('id, name').eq('model_id', model.id).eq('is_active', true).order('name')
      .then(({ data }) => { setDbVariants(data ?? []); setLoadingVariants(false); });
  }, [form.model_name, dbModels]);

  const set = (k: keyof FormData, v: unknown) => setForm(f => ({ ...f, [k]: v }));
  const setBrand = (v: string) => setForm(f => ({ ...f, brand_name: v, model_name: '', variant_name: '' }));
  const setModel = (v: string) => setForm(f => ({ ...f, model_name: v, variant_name: '' }));
  ...
  const canNext = () => {
    if (step === 1) return !!(form.brand_name && form.model_name && form.year && form.condition && form.assembly && form.price && form.registration_year);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        user_id: user.id,
        brand_name: form.brand_name, model_name: form.model_name, variant_name: form.variant_name || null,
        year: parseInt(form.year), condition: form.condition, assembly: form.assembly,
        body_type: form.body_type || null, color: form.color || null,
        ...
        seller_name: form.seller_name || null, seller_phone: form.seller_phone,
        images: uploadedUrls, status: 'pending',
        title: `${form.year} ${form.brand_name} ${form.model_name}${form.variant_name ? ' ' + form.variant_name : ''}`,
      };
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Select value={form.brand_name} onValueChange={setBrand} disabled={loadingBrands}>
                  <SelectTrigger className="h-10">
                    {loadingBrands ? <span className="text-muted-foreground text-sm flex items-center gap-1.5"><Loader2 className="w-3.5 h-3.5 animate-spin" />Loading…</span> : <SelectValue placeholder="Select brand" />}
                  </SelectTrigger>
                  <SelectContent>
                    {dbBrands.length === 0 && !loadingBrands && <SelectItem value="_none" disabled>No brands available</SelectItem>}
              ...
              <div className="space-y-1.5">
                ...
                <Select value={form.model_name} onValueChange={setModel} disabled={!form.brand_name || loadingModels}>
                  <SelectTrigger className="h-10">
                    {loadingModels ? <span className="text-muted-foreground text-sm flex items-center gap-1.5"><Loader2 className="w-3.5 h-3.5 animate-spin" />Loading…</span> : <SelectValue placeholder={form.brand_name ? 'Select model' : 'Pick brand first'} />}
                  </SelectTrigger>
                  <SelectContent>
                    {dbModels.length === 0 && !loadingModels && form.brand_name && <SelectItem value="_none" disabled>No models for this brand</SelectItem>}
              ...
              <div className="space-y-1.5">
                ...
                <Select value={form.variant_name} onValueChange={v => set('variant_name', v)} disabled={!form.model_name || loadingVariants}>
                  <SelectTrigger className="h-10">
                    {loadingVariants ? <span className="text-muted-foreground text-sm flex items-center gap-1.5"><Loader2 className="w-3.5 h-3.5 animate-spin" />Loading…</span>
                      : <SelectValue placeholder={!form.model_name ? 'Pick model first' : dbVariants.length ? 'Select variant' : 'No variants listed'} />}
                  </SelectTrigger>
                  <SelectContent>
                    {dbVariants.length === 0 && !loadingVariants && form.model_name && <SelectItem value="_none" disabled>No variants for this model</SelectItem>}
              ...
              <div className="space-y-1.5">
                ...
                <Select value={form.year} onValueChange={v => set('year', v)}>
                  ...
                  <SelectContent>{Array.from({ length: 30 }, (_, i) => 2025 - i).map(y => <SelectItem key={y} value={String(y)}>{y}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5">
          ...
          <div className="space-y-5">
            ...
            <Card>
              <CardContent className="p-5 space-y-4">
                ...
                <div>
                  <p className="text-xl font-bold">{form.year} {form.brand_name} {form.model_name} {form.variant_name}</p>
                ...
                <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-sm">
                  {[
                    ...
                    ['Body Type', form.body_type], ['Color', form.color],
