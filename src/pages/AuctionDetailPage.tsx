...
export default function AuctionDetailPage() {
  ...
  useEffect(() => {
    if (!id) return;
    const channel = supabase.channel(`bids-${id}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'auction_bids', filter: `auction_id=eq.${id}` },
        (payload) => {
          ...
          if (b.user_id !== user?.id) toast.info(`New bid: ${formatCurrency(b.amount)}`);
        }
      ).subscribe();
  ...
  async function placeBid() {
    ...
    setPlacing(true);
    try {
      const { error } = await supabase.from('auction_bids').insert({ auction_id: id, user_id: user.id, amount });
      if (error) throw error;
      await supabase.from('auctions').update({ current_bid: amount, current_price: amount }).eq('id', id).lt('current_bid', amount);
  ...
  async function setProxyBid() {
    ...
    try {
      await supabase.from('proxy_bids').upsert({ auction_id: id, user_id: user.id, max_amount: amount, is_active: true }, { onConflict: 'auction_id,user_id' });
                    ...
                    <CardContent className="p-0">
                      {loadingBids ? (
                        ...
                        <div className="divide-y divide-border max-h-80 overflow-y-auto">
                          {bids.map((bid, i) => (
                            <div key={bid.id} className={cn('flex items-center justify-between px-4 py-3 transition-colors', i === 0 && 'bg-gold/5', bid.is_own && 'bg-blue-50 dark:bg-blue-950/20')}>
                              <div className="flex items-center gap-3">
                                <div className={cn('w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold', i === 0 ? 'bg-gold text-gold-foreground' : 'bg-secondary text-foreground')}>{i + 1}</div>
                                <div>
                                  <p className={cn('text-sm font-medium', bid.is_own && 'text-blue-600 dark:text-blue-400')}>{bid.bidder_name}</p>
                                  <p className="text-xs text-muted-foreground">{new Date(bid.created_at).toLocaleString()}</p>
                                </div>
                              </div>
                              <p className={cn('text-sm font-bold tabular-nums', i === 0 ? 'text-gold' : 'text-foreground')}>{formatCurrency(bid.amount)}</p>
                            </div>
                          ))}
                        </div>
                      )}
