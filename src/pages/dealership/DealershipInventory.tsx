...
import { Plus, Edit, Trash2, Search, Sparkles, Loader2, Eye, X, Image as ImageIcon } from 'lucide-react';
...
export default function DealershipInventory() {
  ...
  useEffect(() => {
    if (user) {
      supabase.from('dealership_staff').select('dealership_id').eq('user_id', user.id).maybeSingle()
  ...
  const handleBrandChange = (brandId: string) => {
    ...
    setForm(prev => ({ ...prev, brand_id: brandId, model_id: '', variant_id: '', model_name: brand?.name || '', variant_name: '' }));
  };

  const handleModelChange = (modelId: string) => {
    const model = models.find(m => m.id === modelId);
    setForm(prev => ({ ...prev, model_id: modelId, variant_id: '', model_name: model?.name || prev.model_name, variant_name: '' }));
  };

  const handleVariantChange = (variantId: string) => {
    const variant = variants.find(v => v.id === variantId);
    setForm(prev => ({ ...prev, variant_id: variantId, variant_name: variant?.name || '' }));
  };
  ...
  const autoTitle = () => {
    const parts = [
      ...
      variants.find(v => v.id === form.variant_id)?.name,
    ].filter(Boolean);
  ...
  const generateDescription = async () => {
    ...
    const variantName = variants.find(v => v.id === form.variant_id)?.name || '';
    await new Promise(r => setTimeout(r, 800));
    const desc = `${form.year} ${brandName} ${modelName} ${variantName} in ${form.color || 'excellent'} color with ${form.mileage ? formatMileage(parseInt(form.mileage)) : 'low mileage'}. ${form.transmission} transmission, ${form.fuel_type} engine. Well-maintained vehicle located in ${form.city || 'Pakistan'}. ${form.condition === 'new' ? 'Brand new, zero meter.' : 'Single owner, all genuine.'} Contact for test drive.`.trim();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        title: form.title || `${form.year} ${brand?.name} ${model?.name || ''}`.trim(),
        brand_id: form.brand_id || null,
        model_id: form.model_id || null,
        variant_id: form.variant_id || null,
        brand_name: brand?.name || '',
        model_name: model?.name || form.model_name,
        variant_name: variant?.name || form.variant_name,
        year: Number(form.year),
        reg_year: Number(form.reg_year),
            ...
            <TabsContent value="basic" className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={form.model_id} onValueChange={handleModelChange} disabled={!models.length}>
                    <SelectTrigger className="h-9 text-sm"><SelectValue placeholder={models.length ? "Select model" : "Select brand first"} /></SelectTrigger>
                    <SelectContent className="max-h-60">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={form.variant_id} onValueChange={handleVariantChange} disabled={!variants.length}>
                    <SelectTrigger className="h-9 text-sm"><SelectValue placeholder={variants.length ? "Select variant" : "Select model first"} /></SelectTrigger>
                    <SelectContent className="max-h-60">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={String(form.reg_year)} onValueChange={v => setForm(p => ({ ...p, reg_year: parseInt(v) }))}>
                    <SelectTrigger className="h-9 text-sm"><SelectValue /></SelectTrigger>
                    <SelectContent className="max-h-52">
