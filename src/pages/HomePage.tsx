...
function HeroSection() {
  ...
  return (
    <section className="relative min-h-[92vh] flex flex-col justify-center overflow-hidden bg-black">
      ...
      <div className="absolute inset-0 pointer-events-none" aria-hidden="true"
        style={{ background: `linear-gradient(to bottom, rgba(0,0,0,${currentOverlay * 0.7}) 0%, rgba(0,0,0,${currentOverlay}) 60%, rgba(0,0,0,${Math.min(currentOverlay + 0.15, 0.95)}) 100%)` }} />

      {/* Gold accent vignette */}
      <div className="absolute inset-0 pointer-events-none" aria-hidden="true"
        style={{ background: 'radial-gradient(ellipse 120% 80% at 80% 20%, hsl(38 90% 44% / 0.12) 0%, transparent 60%)' }} />
      ...
      <div className="absolute top-0 left-0 right-0 h-px pointer-events-none" aria-hidden="true"
        style={{ background: 'linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.08) 30%, rgba(255,255,255,0.18) 50%, rgba(255,255,255,0.08) 70%, transparent 100%)' }} />
          ...
          {slideshowImages.map((_, i) => (
            <button key={i} onClick={() => setSlideIdx(i)}
              className={cn('rounded-full transition-all duration-300', i === slideIdx % slideshowImages.length
                ? 'w-5 h-1.5 bg-[hsl(var(--gold))]' : 'w-1.5 h-1.5 bg-white/30 hover:bg-white/60')}
              aria-label={`Slide ${i + 1}`} />
          ))}
        </div>
      )}

      <div className="relative max-w-7xl mx-auto px-4 md:px-8 pt-32 pb-24 w-full">
        {/* Auction live badge — only shown when auctions feature is ON */}
        {auctionsOn && (
          <div className="inline-flex items-center gap-2 border border-white/12 bg-white/5 backdrop-blur-sm rounded-full px-4 py-1.5 mb-10">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 live-dot shrink-0" />
            <span className="text-[11px] text-white/65 font-medium tracking-widest uppercase">Live Auctions Active</span>
          </div>
        )}

        {/* Headline */}
        <h1 className="text-[clamp(2.4rem,6vw,4.5rem)] font-bold text-white leading-[1.05] tracking-tight mb-6 text-balance max-w-3xl">
          {heroTitle}
        </h1>
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <p className="text-sm md:text-[15px] text-white/55 max-w-lg leading-relaxed text-pretty">
            {heroSubtitle}
          </p>
        </div>

        {/* Search bar */}
        <div className="flex gap-2 p-1.5 bg-white/8 backdrop-blur-md border border-white/14 rounded-xl max-w-lg mt-8">
        ...
        <div className="flex flex-wrap gap-2 mt-5">
          {['SUV', 'Sedan', 'Hatchback', 'Pickup', 'Luxury', 'Electric'].map(type => (
            <button key={type} onClick={() => navigate(`/inventory?body_type=${type}`)}
              className="text-[11px] text-white/50 hover:text-white border border-white/10 hover:border-white/25 rounded-full px-3.5 py-1 transition-all duration-200 hover:bg-white/6 backdrop-blur-sm">
              {type}
            </button>
          ))}
        </div>
    ...
    <section className="py-12 border-y border-border bg-background overflow-hidden">
      ...
      <div className="relative">
        ...
        <div className="flex gap-4 animate-ticker" style={{ width: 'max-content' }}>
          {doubled.map((b, i) => (
            <Link to={`/inventory?brand_id=${b.id}`} key={`${b.id}-${i}`}
