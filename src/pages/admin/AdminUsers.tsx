...
const ROLE_STYLES: Record<string, string> = {
  ...
  dealer: 'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200',
                    ...
                    <td>
                      <Select value={u.role || 'user'} onValueChange={v => updateRole(u.id, v)}>
                        <SelectTrigger className="h-7 text-xs w-28 border-0 p-0">
                          <Badge className={cn('text-xs cursor-pointer', ROLE_STYLES[u.role || 'user'] || ROLE_STYLES.user)}>{u.role || 'user'}</Badge>
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="admin">Admin</SelectItem>
                          <SelectItem value="dealer">Dealer</SelectItem>
