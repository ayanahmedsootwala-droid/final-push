...
type AddCarForm = {
  ...
  drive_type: string; doors: string; seating_capacity: string; cylinders: string;  // all optional - empty = not set
  // Location
...
export default function AdminInventory() {
  ...
  const handleBrandChange = (brandId: string) => {
    ...
    setForm(prev => ({ ...prev, brand_id: brandId, brand_name: brand?.name || '', model_id: '', model_name: '', variant_id: '', variant_name: '' }));
  };
  const handleModelChange = (modelId: string) => {
    const model = models.find(m => m.id === modelId);
    setForm(prev => ({ ...prev, model_id: modelId, model_name: model?.name || '', variant_id: '', variant_name: '' }));
  };
  ...
  const handleSubmit = async () => {
    if (!form.brand_name || !form.model_name || !form.price || !form.year) {
      toast.error('Brand, model, year and price are required');
      return;
    }
    setSaving(true);
    try {
      const title = `${form.year} ${form.brand_name} ${form.model_name}${form.variant_name ? ' ' + form.variant_name : ''}`;
      const { data: inserted, error } = await supabase.from('cars').insert({
        ...
        registration_year: parseInt(form.registration_year) || parseInt(form.year),
  ...
  const openEditDialog = (car: Car) => {
    ...
    setEditForm({
      ...
      registration_year: String(car.registration_year || ''),
  ...
  const saveEdit = async () => {
    ...
    const payload: Record<string, unknown> = {
      ...
      registration_year: editForm.registration_year ? Number(editForm.registration_year) : null,
  ...
  const toggleContactType = async (car: Car) => {
    ...
    toast.success(`Contact switched to ${next === 'admin' ? 'Admin' : 'Dealer'}`);
  };
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <button onClick={() => toggleContactType(car)}
                                ...
                                {contactType === 'admin' ? <><Building2 className="w-3 h-3" />Admin</> : <><Phone className="w-3 h-3" />Dealer</>}
                              </button>
                            </TooltipTrigger>
                            <TooltipContent side="top" className="text-xs">Click to toggle</TooltipContent>
                          </Tooltip>
                        </td>
                        <td className="px-4 py-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-2">
                ...
                <div>
                  ...
                  <Select value={editContactType} onValueChange={v => setEditContactType(v as 'admin'|'dealer')}>
                    <SelectTrigger className="h-9 text-sm"><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="admin">Admin Contact (default)</SelectItem>
