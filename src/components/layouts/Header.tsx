...
export function Header() {
  ...
  const visibleNavLinks = auctionsOn ? navLinks : navLinks.filter(l => l.href !== '/auctions');
  const visibleCmdItems = auctionsOn ? cmdItems : cmdItems.filter(i => i.href !== '/auctions');
  ...
  return (
    <>
      <header className={cn(
        'fixed top-0 left-0 right-0 z-50 transition-all duration-300 h-[68px]',
        scrolled
          ? 'bg-background border-b border-border shadow-sm'
          : 'bg-gradient-to-b from-black/60 via-black/30 to-transparent border-b border-white/[0.06]'
      )}>
        {/* Top gold accent stripe – only when scrolled */}
        {scrolled && (
          <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-[hsl(var(--gold))] to-transparent opacity-70 pointer-events-none" />
        )}
          ...
          <div className="flex items-center justify-between h-full gap-6">
            ...
            <Link to="/" className="flex items-center gap-3 shrink-0 group">
              ...
              <div className="hidden sm:flex flex-col leading-none">
                <span className={cn(
                  'font-semibold tracking-[-0.01em] text-[15px] transition-colors duration-300',
                  scrolled ? 'text-foreground' : 'text-white drop-shadow-[0_1px_3px_rgba(0,0,0,0.7)]'
                )}>
                  {siteName}
                </span>
                <span className={cn(
                  'text-[9px] font-semibold tracking-[0.18em] uppercase transition-colors duration-300 mt-0.5',
                  scrolled ? 'text-[hsl(var(--gold))]' : 'text-[hsl(var(--gold))] drop-shadow-[0_1px_2px_rgba(0,0,0,0.5)]'
                )}>
                  Premium Autos
                </span>
              </div>
            ...
            <nav className="hidden lg:flex items-center gap-1" aria-label="Main navigation">
              {visibleNavLinks.map(link => (
                <Link key={link.key} to={link.href} className={cn(
                  'relative px-3 py-1.5 text-[13px] font-medium tracking-wide rounded-md transition-all duration-200',
                  isActive(link.href)
                    ? scrolled
                  ...
                  {t(link.key)}
                  {isActive(link.href) && (
                    <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-4 h-[2px] rounded-full bg-[hsl(var(--gold))]" />
                  )}
                </Link>
              ))}
            </nav>
            ...
            <div className="flex items-center gap-0.5">
              ...
              <Button variant="ghost" size="sm" onClick={() => setCmdOpen(true)}
                ...
                <kbd className={cn(
                  ...
                  scrolled ? 'bg-muted text-muted-foreground' : 'bg-white/15 text-white/70'
                )}>⌘K</kbd>
              </Button>
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" className={cn(
                      ...
                      scrolled ? 'hover:bg-secondary/80' : 'hover:bg-white/10'
                    )}>
                      ...
                      <ChevronDown className={cn('w-3 h-3 hidden md:block', scrolled ? 'text-muted-foreground' : 'text-white/70')} />
                    </Button>
                  </DropdownMenuTrigger>
                ...
                <div className="hidden md:flex items-center gap-1.5 ml-1">
                  ...
                  <Button size="sm" onClick={() => navigate('/register')}
                    className={cn(
                      ...
                      !scrolled && 'bg-[hsl(var(--gold))] hover:bg-[hsl(var(--gold)/0.85)] text-black shadow-[0_0_14px_rgba(196,147,24,0.35)]'
                    )}>
                    {t('register')}
                  </Button>
                </div>
              ...
              <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
                <SheetTrigger asChild>
                  <Button variant="ghost" size="icon"
                    className={cn('lg:hidden w-8 h-8 ml-0.5 rounded-lg', scrolled ? 'text-foreground' : 'text-white')}>
